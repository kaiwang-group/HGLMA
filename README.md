...
- The folder ```Data/Dataset/BiGG dataset``` contains 108 BiGG GEMs, which are also available in other sources ([download link](http://bigg.ucsd.edu/models)).
- The folder ```Data/Dataset/Taxonomy dataset``` contains 24 draft GEMs, where are generated by CarveMe from the 24 bacterial genomes ([download link](https://www.ncbi.nlm.nih.gov/datasets/genome)).
- The folder ```Data/Dataset/Reaction pool``` contains the universal reaction pool, which is also available in other sources ([download link](https://github.com/canc1993/cheshire-gapfilling/tree/main/data/pools)).
