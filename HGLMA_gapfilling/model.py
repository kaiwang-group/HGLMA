...
class Classifier(nn.Module):
    def __init__(
            ...
            d_k,
            d_v,
            node_embedding,  # 初始可以是 None 或占位张量
            metabolite_count,
            diag_mask,
            bottle_neck,
            **args):
        ...
        if self.is_tensor_embedding:
            ...
            self.is_tensor_embedding = True  # 默认张量
    ...
    def get_node_embeddings(self, x, return_recon=False):
        ...
        if self.is_tensor_embedding:
            # 如果 node_embedding 是张量，直接索引
            embedded_x = self.node_embedding[x_flat]  # (b * seq_len, bottle_neck)
        else:
            # 如果 node_embedding 是可调用对象（例如 nn.Embedding 或 MultipleEmbedding）
    ...
    def get_embedding_static(self, x):
        ...
        x = self.get_node_embeddings(x)  # 无需修改，直接调用更新后的方法
        dynamic, static, attn = self.encode1(x, x, slf_attn_mask, non_pad_mask)
    ...
    def forward(self, x, mask=None, get_outlier=None, return_recon=False):
        ...
        if mode == 'min':
            ...
            output = output / (mask_sum + 1e-10)  # 添加小值防止除以 0！！！！！！！！！！！！！！！！！！！！！
        elif mode == 'first':
            output = output[:, 0, :]
